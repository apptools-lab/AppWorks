...
import { createNpmCommand, checkPathExists, registerCommand, checkIsAliInternal } from '@iceworks/common-service';
...
export class ActionsProvider implements vscode.TreeDataProvider<ItemData> {
  ...
  private buildQuickItems(): ItemData[] {
    ...
    const debugLabel = i18n.format('extension.iceworksApp.showEntriesQuickPick.runDebug.label');
    const debugItem = this.buildActionItem(
      debugLabel,
      i18n.format('extension.iceworksApp.showEntriesQuickPick.runDebug.detail'),
      'runDev.svg',
      {
    ...
    if (isAliInternal) {
      const publishLabel = i18n.format('extension.iceworksApp.showEntriesQuickPick.DefPublish.label');
      const publishItem = this.buildActionItem(
        publishLabel,
        i18n.format('extension.iceworksApp.showEntriesQuickPick.DefPublish.detail'),
        'publish.svg',
        {
      ...
      const buildLabel = i18n.format('extension.iceworksApp.showEntriesQuickPick.runBuild.label');
      const buildItem = this.buildActionItem(
        buildLabel,
        i18n.format('extension.iceworksApp.showEntriesQuickPick.runBuild.detail'),
        'build.svg',
        {
...
export function createActionsTreeView(context: vscode.ExtensionContext) {
  ...
  registerCommand('iceworksApp.npmScripts.stop', (treeItem: TreeItem) => {
